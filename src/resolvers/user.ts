...
export class UserResolver {
	@Authorized()
	@Query((returns) => User)
	async user(@Arg("id", (type) => ID) id: string, @Ctx() ctx: ContextType): Promise<User> {
	...
	// TODO: Should this be authorized?
	// @Authorized()
	...
	@Mutation()
	requestPasswordReset(@Arg("email") email: string, @Ctx() ctx: ContextType): boolean {
	...
	async resetPassword(
		...
		const token = await ctx.TokenModel.findOne({
			user: { _id: user._id },
			token: input.token,
			type: TOKEN_TYPE.PASS_RESET
		});
